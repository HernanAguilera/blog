<template>
  <form class="space-y-6" @submit.prevent="handleSubmit">
    ...
    <div v-if="availableProviders.length > 0" class="mt-6 grid gap-3" :class="availableProviders.length === 1 ? 'grid-cols-1' : 'grid-cols-2'">
      <button
      ...
      >
        <span class="sr-only">Iniciar sesión con {{ provider.display_name }}</span>
        <!-- Google Icon -->
        <svg v-if="provider.name === 'google'" class="w-5 h-5" viewBox="0 0 24 24">
        ...
        <!-- Facebook Icon -->
        <svg v-else-if="provider.name === 'facebook'" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M20 10c0-5.523-4.477-10-10-10S0 4.477 0 10c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V10h2.54V7.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V10h2.773l-.443 2.89h-2.33v6.988C16.343 19.128 20 14.991 20 10z" clip-rule="evenodd" />
        </svg>
        <!-- Twitter Icon -->
        <svg v-else-if="provider.name === 'twitter'" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
// Social providers
const availableProviders = ref<SocialProvider[]>([]);
...
const loadSocialProviders = async () => {
  try {
    const url = `${config.public.apiBaseUrl}/auth/social/providers`;
    const response = await fetch(url);
    if (response.ok) {
      const data = await response.json();
      if (data.success && data.data.providers) {
        availableProviders.value = data.data.providers;
      }
    }
  } catch (error) {
